...
data$combination <- as.factor(ifelse(str_sub(data$conditions, 3, 4)=="cf", "color_form",
                           ifelse(str_sub(data$conditions, 3, 4)=="dc", "dimension_X",
                                  "dimension_X")))
...
ratings_aggregated %>% filter(combination == "dimension_X") -> ratings_aggregated

# make bar plot for descriptive analysis
p <- ggplot(ratings_aggregated, aes(x=dist, y=prefer_first_1st, fill=relevant_property)) +
  ylab("Preference Rating for Subj. First Ordering")+
  xlab("Sharpness") +
  ...
  my_theme()
p
ggsave(p, file = file.path(plots_dir,"emprical_result.png"), width = 20, height = 10)
