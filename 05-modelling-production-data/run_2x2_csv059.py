"""Run full 2x2 (speaker x semantics) with the §7.10 semantic representation.

csv=0.59, fsv=0.50, wf=0.6856, k=0.5
All models: single alpha, log_beta, tau, delta (reported model structure)

Re-run of run_2x2_best_repr.py (which used the now-superseded csv=0.85,
fsv=0.70, wf=1.0) under the §7.10-validated fixed constants — see memo §7.10:
the joint refit on the contextual model wants colour-semval ≈ 0.59 (a softer
colour predicate), and fsv=0.50 / k=0.5 / wf=0.6856 confirmed best-fixed.
This tests whether the speaker×semantics interaction finding holds under the
corrected representation. Original run_2x2_best_repr.py is kept for the
head-to-head comparison.

Run on server with GPU:
    JAX_PLATFORMS='' XLA_FLAGS='' python run_2x2_csv059.py
"""
import os, time, json
...
# §7.10 fixed-constant set (supersedes the csv=0.85/fsv=0.70/wf=1.0 grid).
CSV, FSV, WF, K = 0.59, 0.50, 0.6856, 0.5
...
for name, jitted_fn, per_dim in MODELS:
    ...
    out_nc = ("./inference_data/mcmc_results_2x2csv059_{}"
...
sep = "=" * 80
print("\n" + sep)
print("2x2 RESULTS (§7.10 repr: csv=0.59, fsv=0.50, wf=0.6856, k=0.5)")
print(sep)
...
with open("2x2_csv059_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nSaved: 2x2_csv059_results.json")
