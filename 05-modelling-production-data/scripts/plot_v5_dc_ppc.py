...
NC = {
    "baseline": "inference_data/mcmc_results_reported_speaker_hier_dc_warmup2000_samples1000_chains4.nc",
...
for i, prop in enumerate(PROPS):
    for j, sharp in enumerate(SHARPS):
        ...
        ax.bar(x - 0.5 * bar_w, cell_bs["model_mean"].values, bar_w, label="baseline (reported)",
...
fig.suptitle("Production PPC — dc subset (N=3196): empirical vs baseline vs ext_v1 vs v5 (F2)", fontsize=12)
fig.tight_layout()
for fmt in ("pdf", "png"):
    out = OUT_DIR / f"production_ppc_barplot_v5_dc_F2.{fmt}"
...
fig.tight_layout()
for fmt in ("pdf", "png"):
    out = OUT_DIR / f"production_correlation_v5_dc_F2.{fmt}"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved: {out}")
plt.close(fig)
...
for thresh in (0.02, 0.05):
    ...
    r_bs, _ = pearsonr(sub["emp_prop"], sub["p_bs"])
    r_v1, _ = pearsonr(sub["emp_prop"], sub["p_v1"])
    r_v5, _ = pearsonr(sub["emp_prop"], sub["p_v5"])
    print(f"Filtered emp >= {thresh}: n = {len(sub)}  R² baseline = {r_bs**2:.3f}  R² ext_v1 = {r_v1**2:.3f}  R² v5 = {r_v5**2:.3f}")
