...
filenames = glob.glob('../data/songshan*.json')

header = ""
data = []

#open output file and initialize csv writer for data
write_file = open('../data/songshan.csv', 'w', newline='')
writer = csv.writer(write_file)

#open output file and initialize csv writer for participant info
write_subj_info_file = open('../data/songshan_subj_info.csv', 'w', newline='')
...
for file in filenames:
    id = re.split("songshan",(re.split("\.json", file)[0]))[1]
    #print(id);
