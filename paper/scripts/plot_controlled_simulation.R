...
settings <- tibble(rule=c('terminal',rep('prefix_k0',4)),support=c('two','two','four','fifteen','fifteen'),parameter_set=c(rep('original_grid',4),'fitted_constants'),order='neutral',setting=c('Terminal, 2 orders','Prefix, 2 orders','Prefix, 4 responses','Prefix, 15 responses','Prefix, 15 responses\nProduction constants'))
main <- summaries %>% inner_join(settings,by=c('rule','support','parameter_set','order')) %>% prepare() %>% mutate(setting=factor(setting,levels=rev(settings$setting)))
stopifnot(nrow(main)==10L,all(complete.cases(main)))
# Selection and unit conversion of frozen values, saved before plotting.
write_csv(main,'data/simulation_controlled_main_figure.csv')
main <- read_csv('data/simulation_controlled_main_figure.csv',show_col_types=FALSE) %>% mutate(setting=factor(setting,levels=rev(settings$setting)),semantics=factor(semantics,levels=semantics_levels))
...
save_csp_pdf(p,'figures/simulation_controlled_comparison.pdf',9.2,5.5)
ggsave('figures/simulation_controlled_comparison.png',p,width=9.2,height=5.5,units='in',dpi=180)
