...
\Cref{sec:experiments} reports the two referential studies: a slider-rating task (\Cref{sec:exp:slider}) and a free-production task (\Cref{sec:exp:production}).
\Cref{sec:modelcomparison} fits the four RSA variants to both datasets and quantifies the contribution of each mechanism using PSIS-LOO.
\Cref{sec:simulation} crosses the same $2 \times 2$ in a parameter sweep to identify which ordering effects already follow from compositional restriction alone.
...
\begin{figure}[H]
  ...
  (\textbf{b})~The RSA model: compositional semantics (context-fixed vs.\ context-updating $\theta$, right-to-left) crossed with speaker architecture (global vs.\ incremental, left-to-right).
  (\textbf{c})~Slider ratings of ordering preference and production proportions by utterance category.
  (\textbf{d})~$2 \times 2$ model comparison (PSIS-LOO).}
  \label{fig:overview}
\end{figure}
...
The interaction is best read as a small context-bound modulation, not as a subjectivity-aligned main effect.
...
\begin{figure}[!ht]
  ...
  \caption{Free-production data. Utterance-type proportions by referential context and size discriminability, with bootstrapped $95\%$ confidence intervals; only utterance types reaching at least $2\%$ in one condition are shown.}
...
These results carry P1 and P2 into unconstrained production: the size-first baseline (P1) is attenuated where colour alone identifies the referent, and the context modulation (P2) emerges more sharply than in the slider task.
...
The data therefore run against the subjectivity-aligned prediction that blurrier size yields a uniformly stronger size-first preference.
The production data amplify the first two patterns relative to the slider data: free production elicits stronger context sensitivity and additionally reveals an over-informativity gradient that is itself sensitive to size blur, with blurrier displays eliciting longer, more redundant strings.
...
We use Pareto-smoothed importance-sampling leave-one-out cross-validation \citep[PSIS-LOO;][]{vehtari2017practical}, reporting differences in expected log pointwise predictive density ($\Delta$ELPD) relative to the best-fitting model together with the difference standard error (dse); a ratio $|\Delta\text{ELPD}|/\text{dse}$ substantially greater than 1 indicates a significant difference in out-of-sample predictive accuracy.
All models were implemented in NumPyro \citep{phan2019composable} and estimated with four MCMC chains using the NUTS sampler \citep{hoffman2014no}; convergence was confirmed via split-$\hat{R} < 1.01$ and effective sample sizes exceeding 400 for all parameters across all models.
\Cref{tab:loo-summary} summarises the full PSIS-LOO comparison; dataset-specific parameter inventories, likelihoods, and sampler settings are introduced with each subsection below.
...
The binary slider response space does not strongly discriminate between sharper and softer colour predicates: the slider posterior for $\nu$ is broad ($0.63$, 94\% HDI $[0.50, 0.82]$), bumps the prior boundary at $0.5$, only just excludes $0.80$, and the four models remain statistically indistinguishable in PSIS-LOO across this range (all $|\Delta\text{ELPD}|/\text{dse} < 1$; full numbers in \Cref{sec:appendix:slider-nu}).
...
As the slider results above demonstrate, a minimal speaker --- a single rationality parameter $\alpha$, the language-model weight $\beta$, and a participant-level offset --- already reproduces the slider condition-level pattern almost perfectly.
Free production is a fifteen-way choice over one-, two-, and three-adjective strings, and warrants a richer speaker.
We extend the incremental speaker with a small set of theoretically motivated components, selected through an explicit parsimony criterion rather than tuned to residuals.

\paragraph{Theoretically motivated components.}
Each addition targets a documented property of production or of the adjective-ordering literature, not a particular residual cell.
In the incremental speaker, size and colour receive separate informativity weights ($\alpha_\text{S}$, $\alpha_\text{C}$) because the two adjective types differ in how reliably they discriminate; the global speaker reasons over complete utterances and therefore uses a single rationality.
A canonical-order penalty ($\lambda_\text{noncanon}$) encodes the cross-linguistically robust colour-before-form tendency \citep{sproat1991cross,cinque1994,scott2002}.
A sufficiency boost ($\lambda_\text{suff}$) and a context-gated form-mention boost ($\lambda_\text{form-mod}$) capture, respectively, the tendency to lead with the property that already individuates the referent and the additional form mention when neither size nor colour suffices.
A graded length bias ($\gamma_\text{base}$, $\gamma_\text{one}$, $\gamma_\text{sharp}$) controls the production of longer or shorter strings, including extra modification under blurry size.
A lapse rate ($\epsilon$) absorbs inattentive responses rather than distorting the pragmatic parameters.
Finally, the determinate feature semantics are softened and anchored: colour and form are given graded denotations (as in the slider model) with empirically calibrated reliabilities.

The parsimony walk floors at ten parameters, with every retained coefficient's $94\%$ HDI excluding zero (\Cref{sec:appendix:selection}).
The production data identify $\nu_\text{colour}$ tightly at $0.59$ $[0.56, 0.62]$; the other three semantic constants are not identifiably moved by the data.

The soft colour predicate is itself a substantive result.
At $\nu_\text{colour} \approx 0.59$ the colour informativity weight is roughly threefold larger and the lapse rate roughly halved relative to a near-deterministic specification.
A near-deterministic colour denotation would suppress the pragmatic weight on colour and force the resulting misfit into uniform noise; the calibrated graded predicate lets the informativity weights be estimated properly and lets far less of the data be explained away as inattention.
The data therefore prefer a colour adjective that is graded rather than categorical, consistent with the relaxed-semantics motivation for $\nu$.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\textwidth]{production_model_comparison_best.pdf}
  \caption{Free-production model comparison: PSIS-LOO $\Delta$ELPD relative to the best model for the parameter-matched $2 \times 2$ (speaker $\times$ semantics), with $\pm 1$ difference standard error.}
  \label{fig:production-loo}
\end{figure}

The resulting production speaker has ten population parameters, the participant hierarchy, and five empirically calibrated constants ($\nu_\text{colour}=0.59$, $\nu_\text{form}=0.50$, $k=0.50$, $w_f=0.69$, and the fixed LM temperature); its full parameter inventory and priors are given in \Cref{tab:param-inventory}.
The four models of the $2 \times 2$ are parameter-matched on this inventory --- identical ten-parameter set, identical participant hierarchy, identical calibrated constants --- so any fit difference between models reflects the architecture and the semantic regime rather than expressive flexibility.
Production responses are categorical over the fifteen utterance types; each chain drew 5{,}000 warmup and 2{,}000 post-warmup samples (target acceptance probability $= 0.85$, maximum tree depth $= 7$).
The advocated production model is the incremental, context-updating variant; it reproduces the production data closely ($R^2 \approx 0.94$, $r = 0.97$; \Cref{fig:production-results-bars} shows the by-condition fit and \Cref{fig:production-results-corr} the condition-level correlation).

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.95\textwidth]{production_ppc_barplot_best.pdf}
  \caption{Posterior-predictive utterance-type proportions for the incremental, context-updating production model. Observed (blue) vs.\ model (red) proportions by referential context $\times$ size discriminability; error bars are $95\%$ bootstrap and posterior-predictive intervals.}
...
\begin{figure}[ht]
  ...
  \caption{Observed vs.\ predicted utterance-type proportions across all condition $\times$ utterance cells for the incremental, context-updating production model; dashed line is identity.}
...
Both incremental models outperform both global models by a large margin: the speaker main effect on out-of-sample fit is $+897$ ELPD, and the global models drop from $R^2 \approx 0.94$ to $R^2 \approx 0.78$.
This effect is not a flexibility artifact.
Under the identical nominal inventory, the effective number of parameters ($p_{\text{LOO}}$) diverges sharply: the incremental models keep the participant hierarchy identifiable ($p_{\text{LOO}} \approx 145$--$175$ of the 113 participant offsets are data-informed), whereas the global models collapse it ($p_{\text{LOO}} \approx 7$; the global speaker is too rigid for the data to inform per-participant rationality).
...
Within the incremental pair, the semantic regime now matters: the context-updating model significantly outperforms the context-fixed model ($\Delta\text{ELPD} = 63$, dse $= 14$, $|\Delta\text{ELPD}|/\text{dse} = 4.5$).
...
The parameter-matched $2 \times 2$ comparison is shown in \Cref{fig:production-loo}, and the model's posterior-predictive fit in \Cref{fig:production-results-bars,fig:production-results-corr}.
...
Within the incremental models, context-updating semantics significantly improves fit for production ($|\Delta\text{ELPD}|/\text{dse} = 4.5$) but not for slider ratings ($|\Delta\text{ELPD}|/\text{dse} = 0.32$).
...
The same response-space asymmetry is reflected in how strongly each dataset identifies the colour reliability $\nu$: production tightly identifies $\nu_\text{colour}$ at $0.59$ ($94\%$ HDI $[0.56, 0.62]$), with substantive consequences for the other parameters, whereas the slider posterior for $\nu$ is broad and bumps the prior boundary ($0.63$, HDI $[0.50, 0.82]$) with negligible ELPD consequences across the $2 \times 2$.
The binary preference response space is therefore informative about speaker architecture but not about the sharpness of the colour predicate, while the 15-way production response space disambiguates both.
...
First, the simulation shows that the global speaker yields a size-first advantage only under recursive semantics; under static semantics the advantage vanishes.
Second, both incremental models outperform both global models in both datasets.
Third, context-updating semantics improves fit in the production data but not in the slider data, and within the production data only in combination with an incremental speaker --- a speaker $\times$ semantics interaction rather than two independent effects.

The cross-task pattern is also informative.
Context-updating composition helps the global models in the slider data, where it is the only available source of order sensitivity, but not in production, where the language-model prior already provides ordering information.
Within the incremental models the pattern reverses: context-updating helps in production but not in slider ratings, where the binary response space does not discriminate between sharper and softer colour predicates.
The benefit of recursive composition therefore appears to depend both on the absence of alternative order-relevant signals and on the richness of the alternative space.

The fitted colour reliability is $\nu_\text{colour} \approx 0.59$, a graded predicate rather than a near-deterministic one.
Under a near-deterministic predicate the model has little room to weight colour informationally, so unexplained variance accumulates in the uniform lapse term and the colour informativity weight collapses.
Under the graded predicate the colour weight is substantially larger and the lapse rate substantially smaller.

The ablation models reported in the Appendix constrain the architecture further.
The LM-only and lookahead variants are worse than the full incremental model, and the RSA-only variant fails PSIS-LOO diagnostics, indicating that both the lexical prior and the pragmatic component contribute to the predictive distribution over the 15-way response space.
...
In the present framework the default ordering arises from the interaction between semantic noise and recursive composition, and context-dependent modulation from the incremental speaker's sensitivity to the current discriminatory landscape.
% =============================================================================
...
\begin{figure}[H]
  ...
  \caption{Production ablation model comparison. $\Delta$ELPD relative to the best model (incremental, context-updating) with difference standard errors; RSA-only ablation excluded (non-finite PSIS-LOO).}
...
The posteriors below are from the reference fit ($\nu_\text{colour}=0.97$), at which every coefficient is documented; under the calibrated $\nu_\text{colour}=0.59$ the four most colour-sensitive coefficients shift to $\alpha_\text{S}\!\approx\!8.0$, $\alpha_\text{C}\!\approx\!3.8$, $\lambda_\text{noncanon}\!\approx\!4.4$, and $\epsilon\!\approx\!0.11$, and every $94\%$ HDI continues to exclude zero.
...
% =============================================================================
\section{Production Model Selection: Parsimony Frontier and Constant Calibration}
\label{sec:appendix:selection}

\paragraph{A parsimony frontier.}
The component set was not chosen by hand.
We fitted the maximal thirteen-parameter model and walked a leave-one-out frontier, dropping coefficients that carry no fit information and stopping at the first removal that did cost fit (\Cref{tab:parsimony-frontier}).
The frontier floor is a ten-parameter model: every retained coefficient has a $94\%$ HDI excluding zero, and removing any of the remaining ten --- $\lambda_\text{suff}$ and $\gamma_\text{sharp}$ in particular --- measurably degrades fit.
We report this ten-parameter model, with the maximal model and the frontier itself as the selection evidence.
...
The ten-parameter model still depends on four constants held fixed during inference: the colour and form reliabilities $\nu_\text{colour}$, $\nu_\text{form}$, the size-threshold anchor $k$, and the size-sigmoid sharpness $w_f$.
We audited all four jointly (\Cref{tab:semval-refit}).
A static sweep over one constant is misleading because the constant trades off against the fitted parameters; only a joint refit licenses a conclusion about its value.
Under joint refit, only $\nu_\text{colour}$ warranted a changed value, moving from the originally assumed $0.97$ to $0.59$ $[0.56, 0.62]$; re-fixing it at $0.59$ recovers the gain at ten parameters with clean sampling.
The other three constants did not move identifiably, and freeing all four jointly produced a non-identified ridge, which we take as a caution against over-parameterising the semantics.
...
Production already imposes a calibration step that lets the data choose $\nu_\text{colour}$ (\Cref{sec:model:extended}); for parity we apply the same audit to the slider's incremental models.
...
First, $\nu$ is weakly identified in the slider data: the posterior bumps the prior lower wall at $0.5$ and only just excludes the fixed value at the upper end, in contrast to the tight production posterior $0.59\;[0.56, 0.62]$.
...
Third, under the freed posterior the rationality parameter $\alpha$ moves to roughly $1.5$ (from $0.69$) and the bias intercept $b$ moves below $0.5$ (from $0.58$), exactly because $\nu$ and the other parameters trade off: a softer colour predicate forces a stronger rationality term and reduces the residual size-first bias absorbed by $b$.
...
The cross-dataset asymmetry --- production strongly identifies $\nu$ at a value with substantive parameter consequences, the slider does not --- is itself diagnostic of the response-space sensitivity argued for in the Cross-Dataset Summary.
