...
\begin{itemize}\itemsep0.15em
  ...
  \item \main{Full-data generalization}: v5 F2 fit on all $N=9586$ gives $+4862$ ELPD over ext-v1 (vs $+1093$ on dc subset) --- $\sim\!4.5\times$ larger gap. $R^2 = .78$ on 90 marginalised cells.
  \item $\mu_\mathrm{noncanon}$ posterior: $-5.08$ (dc) $\to$ $-8.30$ (full data) --- stronger canonical-ordering penalty needed on the full 9-condition data.
\end{itemize}
\end{frame}
...
% ====================================================================
\begin{frame}{PPC --- v5 (F2) fit on \emph{full data}, displayed on dc cells}
\vspace{-0.5em}
\begin{center}
\includegraphics[width=0.98\linewidth]{production_ppc_barplot_v5_fullfit_dc_F2.pdf}
\end{center}
\vspace{-0.3em}
\footnotesize Fit on $N = 9586$ (all 9 conditions); display on dc only ($N = 3196$) to remain comparable with the slider-data figure. Rows: \texttt{relevant\_property} (\acct{first} / \accg{both} / \accy{second}). Cols: sharpness. $R^2$ on display = $.84$.
\end{frame}

% ====================================================================
\begin{frame}{Same fit, displayed on all 9 conditions}
\vspace{-0.5em}
\begin{center}
\includegraphics[width=0.92\linewidth]{production_ppc_barplot_v5_fullfit_all9_F2.pdf}
\end{center}
\vspace{-0.3em}
\footnotesize 9 conditions $\times$ 2 sharpness $= 18$ panels. v5 F2 (gold) tracks empirical across dc, df, and cf families. $R^2$ on display $= .73$ (270 cells); baseline on same view $= .10$.
\end{frame}
...
% ====================================================================
\begin{frame}{Parallel exploration: mixture-of-speakers}
\small
A separate thread (memo \S6 open question \#2, earlier 2026-03): \emp{strategy heterogeneity}. Two-component mixture, each an incremental recursive speaker with its own $\alpha$ and $\gamma$.

\vspace{0.4em}
\begin{itemize}\itemsep0.15em
  \item Mixing weight $\pi \sim \mathrm{Beta}(2,2)$, lapse $\varepsilon \sim \mathrm{Beta}(2,98)$.
  \item Label-switch fix: $\mathrm{gap}_D \sim \mathrm{HalfN}(3)$, $\alpha_{2,D} = \max(\alpha_{1,D} - \mathrm{gap}_D, 0)$.
  \item Two variants: \main{per-dim $\alpha$} (9 per-component params) and \main{simple single-$\alpha$} (2 per-component params).
\end{itemize}

\vspace{0.6em}
\main{Most informative run: per-dim mixture, FULL data (N=9586)}
\begin{center}\footnotesize
\begin{tabular}{lrr|l}
\toprule
...
$\alpha_C$ & \main{$20.85$} & \emp{$1.24$} & \main{speaker 1 super-rational on colour; 2 barely} \\
$\alpha_F$ & $6.79$  & $1.29$ & speaker 1 deeper on form also \\
$\gamma$   & \main{$-2.29$} & \emp{$+1.76$} & \main{1: penalises length; 2: rewards length} \\
...
\vspace{0.5em}
\emp{Caveat.} All mixture runs show $\hat R \in [1.4, 2.8]$ despite $0$ divergences. NUTS finds the two modes but chains don't mix.
\end{frame}

% ====================================================================
\begin{frame}{Mixture takeaways and unresolved issues}
\small
\main{The qualitative story works:}
\begin{itemize}\itemsep0.15em
  \item Two sub-populations \emph{fall out of the data} cleanly in the long run.
  \item \main{Speaker 1} ($\alpha_C = 20.9$, $\gamma = -2.3$): utility-reasoning, minimum-description style.
  \item \main{Speaker 2} ($\alpha_C = 1.2$, $\gamma = +1.76$): low-$\alpha$, mention-more default.
  \item This is \emp{exactly the strategy-heterogeneity hypothesis} that ext-v1's high $\varepsilon = 0.23$ was absorbing.
\end{itemize}

\vspace{0.6em}
\main{The quantitative story doesn't yet work:}
\begin{itemize}\itemsep0.15em
  \item Every variant: $\hat R \geq 1.4$ on key parameters; posteriors are multi-modal / weakly identified.
  \item On the subset (short run) components don't even separate ($\mathrm{gap}_D \approx 0.1$).
  \item Simple single-$\alpha$ variant (5000-warmup long): $\alpha_1$ converges ($\hat R = 1.04$) but $\alpha_2$ ($\hat R = 2.80$) and $\gamma_2$ don't.
\end{itemize}

\vspace{0.6em}
\main{Relation to v5.}
\begin{itemize}\itemsep0.15em
  \item v5 fits a \emph{single-strategy} model with context-dependent mechanisms ($\lambda_C$, $\mu_\mathrm{nc}$, $\delta\gamma$, $\eta$).
  \item Mixture fits a \emph{strategy-heterogeneous} model without those mechanisms.
