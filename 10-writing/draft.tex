...
A joint refit that lets the colour reliability $\nu$ vary --- the same audit that, for the production model, moved $\nu_\text{colour}$ to $0.59$ and substantially shifted the size and colour weights (\Cref{sec:model:extended}) --- here produces a posterior for $\nu$ that is weakly identified and broad, with mean $0.63$ and 94\% HDI $[0.50, 0.82]$ that hits the prior wall at $0.5$ and only just excludes the fixed value $0.80$ (full numbers in \Cref{sec:appendix:slider-nu}).
The four cells are statistically indistinguishable in PSIS-LOO (all $|\Delta\text{ELPD}|/\text{dse} < 1$ once the free-$\nu$ variants are included), confirming that the binary slider response space does not strongly discriminate between sharper and softer colour predicates.
Under the freed posterior the rationality $\alpha$ and the bias $b$ shift substantially (towards $1.5$ and below $0.5$, respectively), so the positive-bias finding above should be read as conditional on the fixed $\nu = 0.80$ specification rather than as a structurally robust quantity.
...
Production already imposes a calibration step that lets the data choose $\nu_\text{colour}$ (\Cref{sec:model:extended}); for parity we apply the same audit to the slider's incremental cells.
...
Second, the four cells are statistically indistinguishable in PSIS-LOO ($|\Delta\text{ELPD}|/\text{dse} < 1$ throughout), so out-of-sample fit cannot decide between sharper and softer colour predicates in this response space.
