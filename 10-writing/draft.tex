...
For the slider-rating models, the semantic parameters $k = 0.5$, $w_f = 1.0$, $\nu_\text{colour} = 0.85$, and $\nu_\text{form} = 0.70$ are fixed across all variants.
...
The $2 \times 2$ contrast isolates two mechanisms, but the minimal speaker it uses --- a single rationality parameter $\alpha$, the language-model weight $\beta$, and a participant-level offset --- is deliberately austere.
For the binary slider task this minimal model already reproduces the condition-level pattern almost perfectly (\Cref{sec:modelcomparison}).
For free production over fifteen surface forms it does not: its best variant explains only about 40\% of the cell-level variance (\Cref{sec:appendix:simple2x2}).
A model we are willing to advocate as a process account of production should fit the production data well, and should do so with a parameter set that is motivated and selected by an explicit criterion rather than hand-tuned to residuals.
We therefore extend the incremental speaker with a small number of independently motivated components and then prune them with a parsimony frontier and a fixed-constant sensitivity analysis.
...
This is a methodological lesson worth stating explicitly: a static parameter sweep can be actively misleading when parameters trade off, and only a joint refit licenses a conclusion about a fixed constant.
Re-fixing $\nu_\text{colour}$ at the freed posterior mean ($0.59$) recovers the entire gain at ten parameters with clean sampling, strictly dominating both the original value and freeing the constant.
The other three constants are kept fixed: freeing each alone gains essentially nothing, the form reliability is unidentified, the sharpness brackets its fixed value, and freeing all four jointly produces a non-identified ridge (effective sample sizes in the single digits) --- a concrete caution against over-parameterising the semantics.

The soft colour predicate is itself theoretically informative.
...
The data prefer a colour adjective that is graded, not categorical --- consistent with the relaxed-semantics motivation for $\nu$ in the first place, and a sharper version of that claim than the assumed value encoded.
...
Both incremental cells outperform both global cells by an enormous margin: the speaker main effect on out-of-sample fit is $+897$ ELPD, and the global cells collapse from $R^2 \approx 0.94$ to $R^2 \approx 0.78$.
This effect is not a flexibility artifact.
Under the identical nominal inventory, the effective number of parameters ($p_{\text{LOO}}$) diverges sharply: the incremental cells keep the participant hierarchy identifiable ($p_{\text{LOO}} \approx 150$--$175$ of the 113 participant offsets are data-informed), whereas the global cells collapse it ($p_{\text{LOO}} \approx 7$; the global speaker is too rigid for the data to inform per-participant rationality).
...
This interaction is robust on the advocated architecture; in the deliberately austere $2 \times 2$ of \Cref{sec:appendix:simple2x2} it is masked, which we show there is a property of that model's impoverishment rather than of the phenomenon.
...
Third, context-updating semantics and incremental production interact --- context-updating composition adds significant predictive value for production, but only when paired with an incremental speaker, and this interaction is robust on the model we advocate.
...
A methodological caveat sharpens this reading.
In a deliberately austere production model (\Cref{sec:appendix:simple2x2}), the within-incremental advantage of context-updating semantics collapses to a tie.
On the model we advocate --- which adds independently motivated structure and a calibrated colour semantics --- the same contrast is decisive ($|\Delta\text{ELPD}|/\text{dse}=4.5$) at the identical colour-reliability setting.
The apparent fragility of the recursive benefit is therefore a property of the impoverished model, not of the phenomenon: recursive composition is a genuine synergistic enhancer of incremental production, and conclusions about ``when recursion matters'' must be drawn from a model rich enough to express it, not from the simplest comparison that happens to isolate the speaker contrast.

A second methodological point concerns how the speaker $\times$ semantics comparison was made defensible, and we state it because it generalises beyond this study.
...
This makes the speaker effect conservative rather than inflated.
We therefore treat the speaker $\times$ semantics \emph{interaction}, not a raw fit ranking, as the inferential target, and we test its robustness across the semantic representation: an effect that survives a change in the colour-reliability assumption (incremental $\gg$ global) is a strong claim, whereas one whose visibility depends on model richness (recursive $>$ static) is reported as the conditional finding it is.

The calibration of the colour semantics is itself substantive, not merely a tuning step.
...
This is a sharper form of the relaxed-semantics hypothesis that motivated graded feature denotations in the first place, and it carries a general lesson --- a one-dimensional sweep over a fixed constant, holding the fitted parameters at their old values, pointed in exactly the wrong direction; only a joint refit that lets the other parameters readjust revealed the preferred value.
...
% =============================================================================
\section{The Austere $2 \times 2$ and Representation Sensitivity}
\label{sec:appendix:simple2x2}

The main-text production comparison is run on the extended model.
For completeness and as a robustness check we also report the deliberately austere $2 \times 2$ --- a single rationality parameter $\alpha$, the language-model weight $\beta$, the hierarchical spread $\tau$, and per-participant offsets $\delta_i$ (116 nominal parameters per cell, identical across cells) --- and its sensitivity to the assumed colour reliability $\nu_\text{colour}$.
This model explains only about $40\%$ of the cell-level production variance, but it isolates the speaker and semantics contrasts with the minimum machinery and so is a useful stress test of the qualitative conclusions.

\Cref{tab:repr-sensitivity} reports the four austere cells at three colour-reliability settings.
Two conclusions follow.
First, the dominant speaker effect is representation-robust: incremental $\gg$ global at every setting, with a speaker main effect of $+740$ ELPD at $\nu_\text{colour}=0.85$ and still $+494$ at $\nu_\text{colour}=0.59$ (always $|\Delta\text{ELPD}|/\text{dse} \gg 10$).
Second, the within-incremental advantage of context-updating semantics is \emph{not} representation-robust in this austere model: it is decisive at $\nu_\text{colour}=0.85$ ($\Delta\text{ELPD}=15.6$, $|\Delta\text{ELPD}|/\text{dse}=10.1$) but collapses to a tie at $\nu_\text{colour}=0.59$ ($|\Delta\text{ELPD}|/\text{dse}=1.1$).
The main text shows that this fragility is an artifact of the austere model: on the advocated architecture the same contrast is decisive at $\nu_\text{colour}=0.59$.
The colour-reliability value calibrated for the extended model therefore does not transfer to the austere one; the austere $2 \times 2$ is reported at $\nu_\text{colour}=0.85$, the value at which it best preserves both the speaker effect and the recursive contrast.

The realized-complexity asymmetry also holds in the austere model: at an identical 116-parameter nominal inventory, the incremental cells retain an estimable participant hierarchy ($p_{\text{LOO}}\approx 70$--$99$) while the global cells collapse it ($p_{\text{LOO}}\approx 3$--$23$), so the speaker advantage is conservative there too.

\begin{table}[htbp]
\centering
\caption{Austere $2 \times 2$ (single-$\alpha$) production cells at three colour-reliability settings.
ELPD$_{\text{LOO}}$ is the absolute expected log pointwise predictive density; $p_{\text{LOO}}$ is the effective number of parameters at $\nu_\text{colour}=0.59$.
The speaker effect is robust across settings; the within-incremental recursive advantage is present at $0.85$ and gone at $0.59$.}
\label{tab:repr-sensitivity}
\small
\begin{tabular}{lrrrr}
\hline
Cell & ELPD$_{0.97}$ & ELPD$_{0.85}$ & ELPD$_{0.59}$ & $p_{\text{LOO},0.59}$ \\
\hline
Incremental, context-updating & $-7{,}180$ & $-6{,}985$ & $-7{,}532$ & 99.2 \\
Incremental, context-fixed & $-7{,}199$ & $-7{,}001$ & $-7{,}533$ & 99.1 \\
Global, context-updating & $-7{,}628$ & $-7{,}728$ & $-8{,}027$ & 3.7 \\
Global, context-fixed & $-7{,}626$ & $-7{,}739$ & $-8{,}027$ & 3.1 \\
\hline
\end{tabular}
\end{table}
