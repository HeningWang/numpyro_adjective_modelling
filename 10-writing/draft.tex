...
To answer these questions, we ran a full factorial sweep over the parameters listed in \Cref{tab:simulation}, crossing both speaker models (incremental, global) with both semantic regimes (recursive, static).
...
This reveals that the global speaker's baseline advantage depends entirely on recursive semantics --- it is not a property of the speaker architecture per se but of the order-sensitive threshold adaptation it exploits.
